...
def create_postgres_tables(pg_conn):
    ...
    create_tables_sql = """
    -- Create User table (user is reserved in PostgreSQL, so we use quotes)
...
def migrate_users(sqlite_conn, pg_conn):
    ...
    try:
        ...
        for user in users:
            pg_cur.execute("""
                INSERT INTO user (name, email, phone, password_hash, is_admin, joined_date, 
                                location, location_enabled, co2_saved, money_saved, trips_taken)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                user['name'], user['email'], user['phone'], user['password_hash'],
                user['is_admin'], user['joined_date'], user['location'],
                user['location_enabled'], user['co2_saved'], user['money_saved'], user['trips_taken']
            ))
...
def migrate_trips(sqlite_conn, pg_conn):
    ...
    try:
        ...
        if not trips:
            print("ℹ️ No trips found in SQLite database")
            return True
        
        # Insert trips into PostgreSQL
        for trip in trips:
            pg_cur.execute("""
                INSERT INTO trip (user_id, destination, mode, distance_km, co2_emitted, cost, timestamp)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """, (
                trip['user_id'], trip['destination'], trip['mode'],
                trip['distance_km'], trip['co2_emitted'], trip['cost'], trip['timestamp']
            ))
...
def main():
    ...
    if not pg_conn:
        ...
        print("   POSTGRES_DB=raahi_db")
        print("   POSTGRES_USER=raahi_user")
        print("   POSTGRES_PASSWORD=raahi_password")
